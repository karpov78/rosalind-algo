...
class FittingAlignmentMatrix(LevMatrix):
    ...
    def calculatePathFromTop(self, top, x, y):
        return top + self.weight(GAP_SYMBOL, None) if y > 0 else 0

    def createCell(self, x, y, weight, prevCell=None):
        if x < len(self.s) or prevCell is None:
            return weight
        if prevCell[0] == x - 1 and prevCell[1] == y - 1 and (
            not self.max_value or self.matrix[self.max_value] < weight):
            self.max_value = (x, y)
        return weight
...
def printAlignment(m, cell):
    ...
    while cell[1] >= 1:
        ...
        elif top + weight(s_char, GAP_SYMBOL) == cell_value:
            s_prefix = s_char + s_prefix
            t_prefix = GAP_SYMBOL + t_prefix
            cell = (cell[0] - 1, cell[1])
        elif left + weight(GAP_SYMBOL, t_char) == cell_value:
    ...
    for i in range(len(s_prefix)):
        res += 1 if s_prefix[i] == t_prefix[i] else -2
    print(res)
